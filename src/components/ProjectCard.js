...
export function ProjectCard(project) {
  ...
  return `<article class="card projectCard">
    <div class="pageHead" style="margin:0"><div><h3 style="margin:0">${project.name}</h3><div class="muted">${project.customerName || 'Заказчик не указан'}</div></div><span class="badge lime">${statusName(project.status)}</span></div>
    ...
    <div class="actions"><button class="btn primary small" data-open="${project.id}">Открыть</button><button class="btn ghost small" data-duplicate="${project.id}">Дублировать</button><button class="btn danger small" data-delete="${project.id}">Удалить</button></div>
  </article>`;
}
