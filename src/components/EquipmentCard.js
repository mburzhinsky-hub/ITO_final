...
export function EquipmentCard(item, settings, library = []) {
  ...
  return `<article class="card itemCard equipmentCard">
    ...
    <div class="tagRow"><span class="badge">${escapeHtml(rootCategoryName(item.categoryId))}</span><span class="badge">${escapeHtml(subcategoryName(item.subcategoryId) || item.category)}</span><span class="badge lime">${escapeHtml(item.solutionLevel || 'standard')}</span>${item.supplier ? `<span class="badge">${escapeHtml(item.supplier)}</span>` : ''}</div>
    <strong>${formatMoney(itemUnitCostRub(item, settings))}</strong>
    <div class="muted smallText">${escapeHtml(item.priceStatus || 'actual')} · ${item.currency === 'USD' ? `$${Number(item.unitCost || 0).toLocaleString('ru-RU')}` : formatMoney(item.unitCost)}</div>
    ...
    <div class="cardActions"><button class="btn primary small" data-add-library="${escapeAttr(item.id)}">Добавить в смету</button><button class="btn ghost small" data-mark-review="${escapeAttr(item.id)}">Пометить проверку</button></div>
  </article>`;
}
