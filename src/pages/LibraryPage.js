...
export function LibraryPage(root) {
  ...
  <div class="separator"></div><div class="libraryGrid">${items.map(item=>`<article class="card itemCard"><div><div class="itemTitle">${item.name}</div><div class="muted">${item.brand} ${item.model}${item.article ? ` · арт. ${item.article}` : ''}</div></div><div><span class="badge">${item.category}</span> <span class="badge">${item.scenario}</span> <span class="badge">${item.currency}/${item.priceMode}</span> ${item.supplier ? `<span class="badge lime">${item.supplier}</span>` : ''}</div><strong>${formatMoney(itemUnitCostRub(item, settings))}</strong><div class="muted">${item.currency === 'USD' ? `$${Number(item.unitCost || 0).toLocaleString('ru-RU')} · по курсу ${settings.usdRate}` : `${formatMoney(item.unitCost)}`}</div><p class="muted">${item.note?.slice(0,160) || ''}</p><button class="btn primary small" data-add-library="${item.id}">Добавить в смету</button></article>`).join('')}</div>`);
